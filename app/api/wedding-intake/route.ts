...
function storeSubmission(data: Record<string, unknown>) {
  try {
    ...
    submissions.push({ ...data, submittedAt: new Date().toISOString(), type: "wedding-intake" });
...
export async function POST(request: NextRequest) {
  ...
  await sendFormEmail({
    subject: `Bruiloft-intake: ${coupleNames}`,
    html: formatFormEmail("Bruiloft Intake", body),
    replyTo: email,
  });
  ...
  if (email) {
    await sendAutoResponder({
      to: email,
      name: coupleNames,
      formType: "wedding-intake",
    });
  }

  return NextResponse.json({
    success: true,
    message: "Bedankt voor het invullen van de bruiloft-intake!",
  });
}
