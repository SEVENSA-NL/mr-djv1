...
async function bootstrap() {
  ...
  // Start competitive intelligence worker
  try {
    await competitiveIntelligenceWorker.start();
    logger.info('Competitive Intelligence Worker started');
  } catch (error) {
    logger.error({ err: error }, 'Failed to start CI Worker');
  }
...
async function shutdown(server) {
  ...
  await Promise.allSettled([
    ...
    competitiveIntelligenceWorker.stop(),
    closeAllQueues(),
