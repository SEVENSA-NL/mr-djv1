...
from faq_release import artifact_digest, build_artifact, canonical_json, source_manifest_digest, validate_artifact  # noqa: E402
...
def decision_digest(decision):
    return hashlib.sha256(canonical_json({key: value for key, value in decision.items() if key != "decision_sha256"}).encode()).hexdigest()
...
class FaqReleaseTests(unittest.TestCase):
    ...
    def test_executor_rejects_wrong_tenant_path_and_missing_rollback_before_network(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            decision = {**{key: self.artifact[key] for key in ("artifact_sha256", "content_sha256", "source_manifest_sha256", "allowlist_sha256", "source_commit", "brand", "tenant", "site", "executor", "rollback_profile")}, "decision": "PUBLISH", "gates": {"all": True}, "review": {"artifact_sha256": self.artifact["artifact_sha256"], "content_sha256": self.artifact["content_sha256"], "score": 90}}
            decision["decision_sha256"] = decision_digest(decision)
            decision_path = root / "decision.json"
            decision_path.write_text(json.dumps(decision), encoding="utf-8")
            kwargs = dict(artifact_path=artifact_path, decision_path=decision_path, gate_root=Path(r"C:\Dev\automations"), image_digest="sha256:" + "a" * 64, prestate_image_digest="sha256:" + "b" * 64, rollback_digest="sha256:" + "c" * 64, deployment="mr-dj-frontend", route="sevensa-admin")
            with patch("faq_release_executor._run_central_verifier", return_value={"status": "PASS", "decision_sha256": decision["decision_sha256"]}):
                self.assertEqual(validate_preflight(**kwargs)["network"], "not_contacted")
                with self.assertRaisesRegex(ValueError, "deployment_not_allowed"): validate_preflight(**{**kwargs, "deployment": "other"})
                with self.assertRaisesRegex(ValueError, "immutable_image_and_rollback_digests_required"): validate_preflight(**{**kwargs, "rollback_digest": ""})
