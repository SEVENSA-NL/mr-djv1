...
TOP_LEVEL_KEYS = {"artifact_type", "schema_version", "artifact_sha256", "brand", "tenant", "site", "target_routes", "faq_items", "source_manifest_sha256", "source_manifest", "content_sha256", "allowlist_sha256", "source_commit", "executor", "rollback_profile"}
...
def content_digest(artifact: dict[str, Any]) -> str:
    return sha256(canonical_json({k: artifact[k] for k in ("brand", "site", "target_routes", "faq_items")}))
...
def _source_manifest(commit: str) -> list[dict[str, str]]:
    ...
    for relative, role in SOURCE_FILES:
        entries.append({"path": relative, "sha256": sha256(_git_blob(commit, relative)), "role": role})
...
def build_artifact(*, allowlist_sha256: str, source_commit: str | None = None, today: date | None = None) -> dict[str, Any]:
    ...
    due = date((today or date.today()).year + 1, (today or date.today()).month, (today or date.today()).day).isoformat()
    source_text = _git_blob(commit, "frontend-nextjs/docs/faq-content-bank-2026-08-21.md").decode("utf-8")
    manifest = _source_manifest(commit)
    refs = [entry["path"] for entry in manifest if entry["role"] in {"faq-content", "canonical-faq-adapter"}]
    items = []
    for index, (question, answer) in enumerate(_read_items(source_text), 1):
        items.append({"id": f"faq-{index}-{_slug(question)}", "question": question, "answer": answer,
                      "provenance_owner": "mr-dj-content", "reviewed_on": reviewed, "review_due": due,
                      "source_refs": refs, "claim_classification": "safe", "fallback": "Neem contact op voor een antwoord op maat."})
    artifact: dict[str, Any] = {
        "artifact_type": "faq_release", "schema_version": "faq-release-artifact-v1", "artifact_sha256": "",
        "brand": "mr_dj", "tenant": "mr_dj", "site": "mr-dj-frontend", "target_routes": [FAQ_ROUTE],
        "faq_items": items, "source_manifest_sha256": source_manifest_digest(manifest), "source_manifest": manifest,
        "content_sha256": "", "allowlist_sha256": allowlist_sha256, "source_commit": commit,
        "executor": "mr_dj_ghcr_faq", "rollback_profile": "ghcr-image-digest-v1",
    }
...
def validate_artifact(artifact: dict[str, Any], *, today: date | None = None) -> dict[str, Any]:
    ...
    if artifact["artifact_type"] != "faq_release" or artifact["schema_version"] != "faq-release-artifact-v1":
    ...
    if not isinstance(manifest, list) or manifest != sorted(manifest, key=lambda e: (e["path"], e["role"], e["sha256"])):
    ...
    if not isinstance(items, list) or len(items) != 30 or len({item.get("id") for item in items}) != 30:
        raise ValueError("faq_items_invalid")
    cutoff = today or date.today()
    for item in items:
        required = {"id", "question", "answer", "provenance_owner", "reviewed_on", "review_due", "source_refs", "claim_classification", "fallback"}
        if set(item) != required or not item["source_refs"] or any(ref not in [e["path"] for e in manifest] for ref in item["source_refs"]):
...
def main() -> int:
    ...
    artifact = build_artifact(allowlist_sha256=args.allowlist_sha256, source_commit=args.source_commit)
    if args.validate:
        validate_artifact(artifact)
    args.output.write_text(json.dumps(artifact, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"artifact": str(args.output), "artifact_sha256": artifact["artifact_sha256"], "faq_count": 30}))
    return 0
